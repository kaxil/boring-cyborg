...
async function verifyTitles (context, config) {
  ...
  if (configKey in config) {
    ...
    const successMessage = verifyTitles.successMessage
    const failureMessage = verifyTitles.failureMessage
    ...
    if (messages.length === 0) {
      ...
      if (regexpMatch == null) {
        ...
        const params = {
          ...
          context: statusTitle,
          state: 'failure',
          description: failureMessage || `Wrong PR title: ${pr.title}`
        }
        context.github.repos.createStatus(context.repo(params))
        valid = false
      }
      ...
      for (const messageIndex in messages) {
        ...
        if (regexpMatch == null) {
          ...
          const params = {
            ...
            context: statusTitle,
            state: 'failure',
            description: failureMessage || `Wrong commit title: ${commitTitle}`
          }
          // Create the status
          context.github.repos.createStatus(context.repo(params))
          valid = false
    ...
    if (valid) {
      ...
      const params = {
        ...
        context: statusTitle,
        state: 'success',
        description: successMessage || 'Validation successful!'
      }
      context.github.repos.createStatus(context.repo(params))
    }
  }
