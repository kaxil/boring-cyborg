...
async function checkUpToDate (context, config) {
  ...
  if (configKey in config) {
    ...
    const getPr = await context.octokit.pulls.get(issue)
    const pr = getPr.data
...
async function _getParentOfFirstCommitOfPR (context, issue) {
  const commits = await context.octokit.pulls.listCommits(issue)
  const firstCommitId = commits.data[0].sha
  const getFirstCommit = await context.octokit.repos.getCommit(context.repo({ commit_sha: firstCommitId }))
  const parentCommitId = getFirstCommit.data.parents[0].sha
  return parentCommitId
}

async function _listBranchesForHeadCommit (context, commit) {
  const getBranches = await context.octokit.repos.listBranchesForHeadCommit(context.repo({ commit_sha: commit }))
...
async function _createStatus (context, pr, checkerName, state, description) {
  ...
  await context.octokit.repos.createStatus(context.repo(params))
}

async function _listFilesInPR (context, issue) {
  const files = await context.octokit.pulls.listFiles(issue)
