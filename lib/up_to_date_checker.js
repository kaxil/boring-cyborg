  ...
  if (configKey in config) {
    ...
    if (modifiedFiles.find(file => matcher.ignores(file))) {
      ...
      if (branchesForHeadCommit.includes('master')) {
        ...
        await _createStatus(context, pr, checkerName, 'success', 'PR is up to date with master')
      } else {
        context.log.info('PR branch is not up to date with master. Need rebase')
        await _createStatus(context, pr, checkerName, 'failure', 'PR is not up to date with master. Please rebase.')
      }
    }
  }
}
