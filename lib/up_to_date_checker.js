...
async function checkUpToDate (context, config) {
  ...
  if (configKey in config) {
    const filePatterns = config[configKey]
    ...
    // Get files in the PR
    const modifiedFiles = _listFilesInPR(context, issue)
    ...
    if (modifiedFiles.find(file => matcher.ignores(file))) {
      ...
      const parentCommitId = _getParentOfFirstCommitOfPR(context, issue)

      const branchesForHeadCommit = _listBranchesForHeadCommit(context, parentCommitId)

      if (branchesForHeadCommit.includes('master')) {
        context.log.info('PR branch is up to date with master')
        _createStatus(context, pr, checkerName, 'success', 'PR is up to date with master')
      } else {
        context.log.info('PR branch is not up to date with master. Need rebase')
        _createStatus(context, pr, checkerName, 'failure', 'PR is not up to date with master. Please rebase.')
      }
    }
  }
}
...
async function _getParentOfFirstCommitOfPR (context, issue) {
  const commits = await context.github.pullRequests.listCommits(issue)
